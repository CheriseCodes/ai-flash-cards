...
import { validateGetImage } from '../helpers/validators';
...
export const postImage = async (req: Request, res: Response) => {
  ...
  const response: GenericServerResponse = await handlePostImage({ cardId: cardId, remoteFileName: remoteFileName, localFileName: localFileName, imgUrl: imgUrl }, dynamoDbClient, s3Client);
  res.status(response.status).send(response.body);
}
