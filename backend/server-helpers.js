...
const validateLangLevel = (langMode, langLevel) => {
    if (langMode == appConfig.languageModes.FRENCH || langMode == appConfig.languageModes.SPANISH) {
        return ["A1","A2","B1","B2","C1","C2"].includes(langLevel)
    } else if (langMode == appConfig.languageModes.KOREAN) {
        return ["TOPIK1", "TOPIK2", "TOPIK3","TOPIK4","TOPIK5","TOPIK6"].includes(langLevel)
    } else {
        return false
    }
}
