...
export const getFlashcardsByUser =  async (req: Request, res: Response) => {
  const response: GenericServerResponse = await handleGetFlashcardsByUser(req, dynamoDbClientInstance);
  res.status(response.status).send(response.body);
}

export const deleteFlashcard =  async (req: Request, res: Response) => {
    const response: GenericServerResponse = await handleDeleteFlashcard(req, dynamoDbClientInstance, s3ClientInstance);
    res.status(response.status).send(response.body);
}
