...
//   }
  ...
  const handleDeletion = () => {
    setAllCardData(prev => prev.filter((prevCardData) => {
  ...
  const handleRegenerateCard = async () => {
    try {
        ...
        console.log(`newCardData JSON - ${generatedCardData.or}, ${generatedCardData.tr}`)
        const newCardData = {id: cardData.id, word: currWord, img: imageJson.data[0].url, or: generatedCardData.or,tr:generatedCardData.tr}
  ...
  return (
    <>
        {!regenerateCardSpinner && <div className={`flash-card ${selected ? 'selected-flashcard' : 'unselected-flashcard'}`}>
                <input ref={wordRef} defaultValue={cardData.word} disabled={enableEdit ? '' : 'disabled'}></input>
                <input ref={originalRef} defaultValue={cardData.or} disabled={enableEdit ? '' : 'disabled'}></input>
                <img src={cardData.img}></img>
                <input ref={translationRef} defaultValue={cardData.tr} disabled={enableEdit ? '' : 'disabled'}></input>
